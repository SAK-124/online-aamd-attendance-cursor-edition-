...
# -*- coding: utf-8 -*-
"""
Zoom Attendance Automator — v4.1 core logic adapted for the serverless API engine.

This module mirrors the processing behaviour of the reference desktop script,
including robust canonicalisation, reconnect detection, alias merging, roster
integration, Excel export with formulas/validation, and exemption handling.
"""
...
PRESET_CSV_DIR = "/Users/sak/Documents/AAMD_ATT"
...
def _merge_intervals(intervals: List[Tuple[pd.Timestamp, pd.Timestamp]]) -> List[Tuple[pd.Timestamp, pd.Timestamp]]:
    ints = [
        ...
        if s is not None and e is not None and pd.notna(s) and pd.notna(e) and e > s
    ]
    if not ints:
        return []
    ...
    for a_s, a_e in A:
        ...
        while cur < a_e and j < len(B):
            ...
            if b_s >= a_e:
                break
            if b_s > cur:
                total += (b_s - cur).total_seconds() / 60.0
            cur = max(cur, b_e)
            if b_e <= cur:
                j += 1
        if cur < a_e:
            total += (a_e - cur).total_seconds() / 60.0
    return total
...
def _detect_columns(df: pd.DataFrame):
    ...
    if name_col is None:
        raise ValueError(
            f"Could not detect participant name column. Found: {list(df.columns)}"
        )
...
def _detect_roster_columns(df: pd.DataFrame):
    ...
    def is_5d(x):
        try:
            return bool(re.match(r"^\s*\d{5}\s*$", str(x)))
        except Exception:
            return False
...
def load_roster(path: str) -> pd.DataFrame:
    ...
    if erp_col is None or name_col is None:
        raise ValueError(
            "Could not detect ERP/Name columns in roster. Make sure it has 5-digit ERP and a Name column."
        )
    out = pd.DataFrame(
        {
    ...
    if email_col:
        ...
        out["Email"] = ""
    out = (
        out.dropna(subset=["ERP", "RosterName"])
        .drop_duplicates(subset=["ERP"], keep="first")
...
def _name_to_erp_and_clean(name: str):
    ...
    if m:
        return m.group(1), m.group(2).strip(), 0
...
def extract_keys_for_ui(csv_path: str) -> List[tuple]:
    ...
    for pat in EXCLUDE_NAME_PATTERNS:
        mask_excl |= df[name_col].astype(str).str.contains(
            pat, flags=re.I, regex=True, na=False
        )
    ...
    def key_of(row):
        if pd.notna(row["_erp"]) and row["_erp"] is not None:
            return f"ERP:{row['_erp']}"
...
def process_zoom_attendance(
    ...
    """
    Implements the full Zoom attendance processing pipeline.
    Mirrors the reference desktop script behaviour including alias merging,
    roster-only absentees, reconnect detection, and Excel export with formulas.
    """
    ...
    df = raw_df.copy()
    cols = _detect_columns(df)
    name_col, join_col, leave_col, duration_col = (
        cols["name_col"],
        cols["join_col"],
        cols["leave_col"],
        cols["duration_col"],
    ...
    has_times = (
        join_col in df
        and leave_col in df
        and df["_join"].notna().any()
        and df["_leave"].notna().any()
    )
    ...
    if override_total_minutes and override_total_minutes > 0:
        ...
        if has_times:
            ...
            if jo.empty or le.empty:
                raise ValueError(
                    "Timestamps present but unparsable. Check the Zoom CSV encoding/format."
                )
            total_class_minutes = float((le.max() - jo.min()).total_seconds() / 60.0)
            total_src = "auto (timestamps)"
        elif duration_col and duration_col in df:
            total_class_minutes = float(
                pd.to_numeric(df[duration_col], errors="coerce").fillna(0).max()
            )
            total_src = "auto (max duration)"
        else:
    ...
    if has_times:
        for _, r in df.iterrows():
            k = r["_key"]
            s, e = r.get("_join", None), r.get("_leave", None)
            ...
            session_records_by_key.setdefault(k, []).append(
                dict(
                    join_ts=s,
                    leave_ts=e,
                    join_raw=r.get(cols["join_col"], ""),
                    leave_raw=r.get(cols["leave_col"], ""),
                    raw_name=r["_rawname"],
                )
            )
            if s is not None and e is not None:
                intervals_by_key[k].append((s, e))
            if r["_pen_flag"] == -1:
                bad_intervals_by_key.setdefault(k, [])
                if s is not None and e is not None:
                    bad_intervals_by_key[k].append((s, e))
            else:
                good_intervals_by_key.setdefault(k, [])
                if s is not None and e is not None:
                    good_intervals_by_key[k].append((s, e))
            if k.startswith("ERP:"):
                match_source_by_key.setdefault(k, "erp_in_name")
            else:
                match_source_by_key.setdefault(k, "name_only")
    else:
        ...
        for _, r in df.iterrows():
            ...
            raw_names_by_key.setdefault(k, set()).add(r["_rawname"])
            if r["_pen_flag"] == -1:
                durations_bad_by_key.setdefault(k, []).append(d)
            else:
                durations_good_by_key.setdefault(k, []).append(d)
            match_source_by_key.setdefault(
                k, "erp_in_name" if k.startswith("ERP:") else "name_only"
            )
    ...
    if has_times:
        ...
        for k, cn in canon_by_key.items():
            if not cn:
                continue
            if k.startswith("ERP:"):
                erp_by_canon.setdefault(cn, []).append(k)
            else:
                name_by_canon.setdefault(cn, []).append(k)
        for cn, nkeys in name_by_canon.items():
            erp_keys = erp_by_canon.get(cn, [])
            if not erp_keys:
                continue
            for name_k in list(nkeys):
                if name_k not in names_by_key:
                    continue
                chosen = None
                if len(erp_keys) == 1:
                    chosen = erp_keys[0]
                    ...
                    for ek in erp_keys:
                        if _intervals_overlap_or_close(
                            name_ints, intervals_by_key.get(ek, []), 7.0
                        ):
                            chosen = ek
                            break
                if chosen is None:
                    ...
                    continue
                intervals_by_key.setdefault(chosen, []).extend(
                    intervals_by_key.get(name_k, [])
                )
                good_intervals_by_key.setdefault(chosen, []).extend(
                    good_intervals_by_key.get(name_k, [])
                )
                bad_intervals_by_key.setdefault(chosen, []).extend(
                    bad_intervals_by_key.get(name_k, [])
                )
                session_records_by_key.setdefault(chosen, []).extend(
                    session_records_by_key.get(name_k, [])
                )
                raw_names_by_key.setdefault(chosen, set()).update(
                    raw_names_by_key.get(name_k, set())
                )
                for dct in (
                    intervals_by_key,
                    good_intervals_by_key,
                    bad_intervals_by_key,
                    names_by_key,
                    erp_by_key,
                    match_source_by_key,
                    raw_names_by_key,
        ...
        for cn, nkeys in name_by_canon.items():
            ...
            if len(erp_keys) == 1:
                ...
                for name_k in list(nkeys):
                    if name_k not in names_by_key:
                        continue
                    durations_good_by_key.setdefault(chosen, []).extend(
                        durations_good_by_key.get(name_k, [])
                    )
                    durations_bad_by_key.setdefault(chosen, []).extend(
                        durations_bad_by_key.get(name_k, [])
                    )
                    raw_names_by_key.setdefault(chosen, set()).update(
                        raw_names_by_key.get(name_k, set())
                    )
                    for dct in (
                        durations_good_by_key,
                        durations_bad_by_key,
                        names_by_key,
                        erp_by_key,
                        match_source_by_key,
                        raw_names_by_key,
    ...
    for k in list(names_by_key.keys()):
        ...
        if has_times:
            ...
            positive_ints = [
                (s, e) for s, e in all_ints if s is not None and e is not None and e > s
            ]
            union_min_raw = _interval_union_minutes(all_ints)
            segments_for_detection = _prepare_segments_for_reconnect(
                session_records_by_key.get(k, [])
            )
            seg_count = (
                len(segments_for_detection)
                if segments_for_detection
                else len(positive_ints)
            )
            reconnect_count, per_key_events = _compute_reconnect_events(
            ...
            bad_only_minutes = _minutes_A_minus_B(
                bad_intervals_by_key.get(k, []),
                good_intervals_by_key.get(k, []),
            )
        else:
            ...
            seg_count = int(
                len(durations_good_by_key.get(k, []))
                + len(durations_bad_by_key.get(k, []))
            )
        ...
        if has_times and per_key_events:
            for ev in per_key_events:
                ...
                gap_seconds = (
                    int(max(0, gap_td.total_seconds()))
                    if isinstance(gap_td, pd.Timedelta)
                    else 0
                )
                ...
                reconnect_rows.append(
                    dict(
                        ...
                        **{
                            "Gap Duration (hh:mm:ss)": _td_to_hms(
                                gap_td if isinstance(gap_td, pd.Timedelta) else pd.Timedelta(0)
                            )
                        },
                        **{
                            "Disconnect Raw Name": _clean_raw_str(
                                disconnect_seg.get("raw_name", "")
                            )
                        },
                        **{
                            "Reconnect Raw Name": _clean_raw_str(
                                reconnect_seg.get("raw_name", "")
                            )
                        },
                        **{
                            "Disconnect Join (raw)": _clean_raw_str(
                                disconnect_seg.get("join_raw", "")
                            )
                        },
                        **{
                            "Disconnect Leave (raw)": _clean_raw_str(
                                disconnect_seg.get("leave_raw", "")
                            )
                        },
                        **{
                            "Reconnect Join (raw)": _clean_raw_str(
                                reconnect_seg.get("join_raw", "")
                            )
                        },
                        **{
                            "Reconnect Leave (raw)": _clean_raw_str(
                                reconnect_seg.get("leave_raw", "")
                            )
                        },
                    )
                )
        ...
        if bool(ex.get("naming", False)):
            pen_applied = 0
        ex_overlap = bool(ex.get("overlap", False))
        ex_reconnect = bool(ex.get("reconnect", False))

        issues = []
        if is_dual and not ex_overlap:
            issues.append("Duplicate account — overlapping (two devices)")
        if is_reconnect and not ex_reconnect:
            if reconnect_count > 0:
                issues.append(
                    f"Duplicate account — reconnects (non-overlapping x{reconnect_count})"
                )
            else:
                issues.append("Duplicate account — reconnects (non-overlapping)")
        if is_amb:
            issues.append("Ambiguous duplicate name (no ERP / alias ambiguous)")
        merges_for_key = [src for (src, dst) in alias_merges if dst == k]
        for src in merges_for_key:
            issues.append(f"Merged alias {src} into {k}")
    ...
    if not roster_df.empty:
        ...
        for _, row in roster_df.iterrows():
            erp = row["ERP"]
            roster_name = row["RosterName"]
            roster_canon = row["RosterCanon"]
            erp_key = f"ERP:{erp}"
            if (
                (erp not in present_erps)
                and (erp_key not in match_source_by_key)
                and (roster_canon not in all_zoom_canon_names)
            ):
                thr_decision = (
                    float(math.ceil(effective_threshold_minutes))
                    if rounding_mode == "ceil_both"
                    else effective_threshold_minutes
                )
                attendance_rows.append(
                    dict(
                        Key=erp_key,
    ...
    issues_df = (
        ...
        if issues_rows
            ...
            columns=[
                ...
                "Reconnect Count",
                "Ambiguous Name?",
                "Total Minutes Counted (Union RAW)",
                "Override Attendance",
            ]
        )
    )
    absent_df = (
        pd.DataFrame(absent_rows)
        if absent_rows
        else pd.DataFrame(
            columns=[
                "Key",
                ...
                "Name",
                "Zoom Names (raw)",
                "Attended Minutes (DECISION)",
                "Threshold Minutes (DECISION)",
                "Shortfall Minutes (DECISION)",
                "Dual Devices?",
                "Reconnects?",
                "Reconnect Count",
                "Is Ambiguous?",
    ...
    penalties_df = (
        ...
        if penalties_rows
            ...
            columns=[
                ...
                "Bad-Name Minutes",
                "Bad-Name %",
                "Penalty Tolerance (min)",
                "Penalty Applied",
            ]
        )
    )
    matches_df = (
        pd.DataFrame(matches_rows)
        if matches_rows
        else pd.DataFrame(
    ...
    reconnects_df = (
        ...
        if reconnect_rows
            ...
            columns=[
                ...
                "Gap (minutes)",
                "Gap (seconds)",
                "Gap Duration (hh:mm:ss)",
                "Disconnect Raw Name",
                "Reconnect Raw Name",
                "Disconnect Join (raw)",
                "Disconnect Leave (raw)",
                "Reconnect Join (raw)",
                "Reconnect Leave (raw)",
            ]
        )
    ...
    meta_rows = [
        ...
        [
            "Raw threshold minutes (ratio * adjusted total)",
            round(threshold_minutes_raw, 2),
        ],
        ["Leniency buffer minutes", round(buffer_minutes, 2)],
        [
            "EFFECTIVE threshold minutes (raw - buffer)",
            round(effective_threshold_minutes, 2),
        ],
        ...
        [
            "Naming penalty tolerance (minutes)",
            float(penalty_tolerance_minutes or 0.0),
        ],
        ["Roster provided", "Yes" if not roster_df.empty else "No"],
        [
            "Excluded names patterns",
            "; ".join(
                [re.sub(r"^\^\s*|\s*\$", "", p) for p in EXCLUDE_NAME_PATTERNS]
            ),
        ],
    ]
    meta_df = pd.DataFrame(meta_rows, columns=["Metric", "Value"])
    summary_df = pd.DataFrame(
        [["(Formulas inserted by app)", ""]], columns=["Metric", "Value"]
    )
        ...
        try:
            with pd.ExcelWriter(output_xlsx, engine=engine) as w:
                ...
                if engine == "openpyxl":
                    ...
                    ws_summary["A1"].value = "Metric"
                    ws_summary["B1"].value = "Value"
                    if absent_df.shape[0] > 0:
                        ah = [c.value for c in next(ws_absent.iter_rows(min_row=1, max_row=1))]
                        ...
                        ws_summary["A2"].value = "Total Absent (final)"
                        ws_summary["B2"].value = 0
                        ws_summary["A3"].value = "Total Needs Review"
                        ws_summary["B3"].value = 0

                    wp = w.sheets["Penalties"]
                    ph = [c.value for c in next(wp.iter_rows(min_row=1, max_row=1))]
...
# -------------------- prefs (retained for compatibility) --------------------
def load_prefs():
    try:
...
def process_request(
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        result = process_zoom_attendance(
            csv_path=zoom_path,
            ...
            exemptions=exemptions or {},
            override_total_minutes=(
                params.get("override_total_minutes")
                if params.get("override_total_minutes") not in (None, "")
                else None
            ),
            penalty_tolerance_minutes=float(
                params.get("penalty_tolerance_minutes", 0.0) or 0.0
            ),
...
def extract_keys_from_bytes(zoom_bytes: bytes) -> List[Dict[str, Optional[str]]]:
    with tempfile.TemporaryDirectory() as td:
        zoom_path = os.path.join(td, "input.csv")
        with open(zoom_path, "wb") as f:
            f.write(zoom_bytes)
        items = extract_keys_for_ui(zoom_path)
    out = []
    for key, erp, name in items:
        erp_clean = None
        if erp is not None and not pd.isna(erp):
            erp_clean = str(erp).strip()
        name_clean = "" if name is None or pd.isna(name) else str(name).strip()
        display = f"{(erp_clean + ' · ') if erp_clean else ''}{name_clean}".strip()
        out.append({"key": key, "erp": erp_clean, "name": name_clean, "display": display})
    return out
